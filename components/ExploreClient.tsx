...
const FESTIVALS_PER_PAGE = 3; // ✅ กำหนดให้แสดงแค่ 3 รูปต่อหน้า เป๊ะๆ
...
export default function ExploreClient({ initialPlaces, searchParams }: ExploreClientProps) {
  ...
  const [festivalPage, setFestivalPage] = useState(1); // ✅ เพิ่ม State หน้าของ Festival
  ...
  // ✅ Festival Logic: Filter & Paginate
  ...
  // Reset Festival Page when Month Changes
  useEffect(() => {
    setFestivalPage(1);
  }, [selectedMonth]);
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] font-inter text-gray-800 pb-20" onClick={() => setShowDropdown(false)}>
      ...
      <style jsx global>{`
        ...
        /* ปุ่มลูกศร Festival (CSS เดิม ใช้ตกแต่งปุ่มเฉยๆ) */
        .festival-nav-btn {
            position: absolute;
            top: 50%;
        ...
        .festival-nav-btn:hover { background-color: #F5F5F5; }
        .festival-nav-btn:disabled { opacity: 0.3; cursor: not-allowed; }
      `}</style>

      {/* Breadcrumb & Hero Slider (Unchanged) */}
      ...
      <div className="max-w-[1440px] mx-auto px-[156px] pt-6 mt-16">
        ...
        <div className="w-[1128px] h-[503px] mx-auto mb-20 flex flex-col gap-[24px] relative"> 
          
          {/* ✅ 1. Relative Wrapper for Slider and Buttons */}
          <div className="w-full relative h-[450px]"> 

            {filteredFestivals.length === 0 ? (
                <div className="w-full h-[441px] flex items-center justify-center text-gray-500 rounded-[24px] bg-[#F5F5F5]">
                    No festivals found for {selectedMonth === "ALL" ? "any month" : selectedMonth} in {paramCountry}.
                </div>
             ) : (
                 <>
                    {/* ✅ Navigation Buttons: Absolute to the wrapper, outside the items */}
                    {totalFestivalPages > 1 && (
                        <>
                            <button onClick={() => setFestivalPage(p => Math.max(1, p - 1))} disabled={festivalPage === 1} className="festival-nav-btn" style={{ left: '-60px' }}>
                                <ChevronLeft size={24} className="text-[#194473]" />
                            </button>
                            <button onClick={() => setFestivalPage(p => Math.min(totalFestivalPages, p + 1))} disabled={festivalPage === totalFestivalPages} className="festival-nav-btn" style={{ right: '-60px' }}>
                                <ChevronRight size={24} className="text-[#194473]" />
                            </button>
                        </>
                    )}

                    {/* ✅ Festival Items Grid (No Swiper, Just Grid + Slice) */}
                    <div className="grid grid-cols-3 gap-[24px]">
                        {currentFestivals.map((festival) => (
                            <div key={festival.id} className="w-[360px] h-[441px] flex flex-col gap-[8px] p-[16px] rounded-[24px] bg-[#F5F5F5] hover:shadow-md transition-shadow">
                                <div className="relative w-[328px] h-[224px] rounded-[8px] overflow-hidden flex-shrink-0">
                                ...
                                <div className="flex flex-col flex-1 overflow-hidden gap-[8px]">
                                    <div className="w-[328px] flex flex-col gap-[4px] overflow-hidden">
                                        <h3 className="font-inter font-bold text-[18px] text-[#212121] leading-tight line-clamp-2">{festival.name}</h3>
                                        <p className="font-inter font-normal text-[16px] text-[#212121] leading-tight line-clamp-4">{festival.about}</p>
                                    </div>
                                    <div className="w-[328px] flex flex-col gap-[4px] mt-auto">
                                        ...
                                        <p className="font-inter font-normal text-[16px] text-[#212121] leading-tight break-words line-clamp-2"><span className="font-bold">Top Spot : </span>{festival.top_spot}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
          ...
          <div className="flex justify-center items-center gap-[8px] mt-auto">
             <button 
               onClick={() => setSelectedMonth("ALL")}
               className={`flex items-center justify-center w-[40px] h-[24px] px-2 rounded-[4px] border border-[#EEEEEE] text-[12px] font-medium transition-colors ${selectedMonth === "ALL" ? "bg-[#194473] text-white" : "bg-[#9E9E9E] text-white hover:bg-gray-400"}`}
             >
               All
             </button>
             {MONTHS.map((month) => (
                <button 
                  ...
                  className={`flex items-center justify-center w-[40px] h-[24px] px-2 rounded-[4px] border border-[#EEEEEE] text-[12px] font-medium transition-colors ${selectedMonth === month ? "bg-[#194473] text-white" : "bg-[#9E9E9E] text-white hover:bg-gray-400"}`}
                >
                  {month}
                </button>
             ))}
