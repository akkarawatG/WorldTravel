...
export default function PlanNewTripForm({ onSuccess }: PlanNewTripFormProps) {
  ...
  return (
    // ✅ เปลี่ยนจาก w-[493px] เป็น w-full max-w-[493px] เพื่อให้ยืดหยุ่นบนมือถือ
    <div className="flex flex-col items-center lg:items-start gap-[8px] w-full max-w-[493px] relative mx-auto lg:mx-0 px-4 sm:px-0">
      <div className="flex flex-col items-center gap-[24px] w-full self-stretch">
        <h2 className="font-inter font-bold text-[28px] sm:text-[36px] leading-[36px] sm:leading-[44px] text-center text-[#000000]">
          Plan a new trip
        </h2>
        <div className="flex flex-col items-start gap-[4px] w-full self-stretch">
          <div className="flex flex-col items-start gap-[16px] w-full self-stretch">
            ...
            <div className="relative w-full" ref={dropdownRef}>
              <div className={`box-border flex flex-row items-center px-[8px] py-[12px] gap-[10px] w-full h-[46px] border ${errorMsg ? 'border-red-500' : 'border-[#E0E0E0]'} rounded-[8px] bg-white`}>
                <input
                  ...
                  className="w-full font-inter font-bold text-[16px] sm:text-[18px] leading-[22px] text-[#000000] outline-none placeholder:text-[#9E9E9E] placeholder:font-normal"
                  disabled={isLoading}
                />
            ...
            <div
              className="box-border flex flex-col items-start px-[12px] py-[8px] w-full h-auto min-h-[50px] border border-[#E0E0E0] rounded-[8px] bg-white relative cursor-pointer hover:border-[#3A82CE] transition"
              onClick={() => setIsDatePickerOpen(true)}
            >
              <span className="font-inter font-bold text-[12px] leading-[15px] text-[#000000] mb-[4px]">
                Dates (optional)
              </span>

              {/* ✅ ใช้ Flex ธรรมดาแทนการใช้ Absolute เพื่อให้เส้นคั่นยืดหยุ่นตามหน้าจอ */}
              <div className="flex flex-row items-center w-full gap-[12px] sm:gap-[24px]">

                {/* Start Date */}
                <div className="flex flex-1 items-end gap-[6px] sm:gap-[8px]">
                  <Calendar className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px] text-[#212121] mb-[1px]" />
                  <span className={`font-inter font-normal text-[11px] sm:text-[12px] leading-[15px] truncate ${startDate ? 'text-[#000000]' : 'text-[#9E9E9E]'}`}>
                ...
                <div className="flex flex-1 items-end gap-[6px] sm:gap-[8px]">
                  <Calendar className="w-[14px] h-[14px] sm:w-[16px] sm:h-[16px] text-[#212121] mb-[1px]" />
                  <span className={`font-inter font-normal text-[11px] sm:text-[12px] leading-[15px] truncate ${endDate ? 'text-[#000000]' : 'text-[#9E9E9E]'}`}>
            ...
            {isDatePickerOpen && (
              ...
              >
                <div className="absolute inset-0 z-[90]" onClick={() => setIsDatePickerOpen(false)}></div>
      ...
      {/* Footer Actions */}
      <div className="flex flex-col items-center justify-center w-full self-stretch mt-6">
        <button
          onClick={handleStartPlanning}
          disabled={isLoading}
          className="box-border flex flex-row justify-center items-center px-[24px] py-[12px] gap-[8px] w-full sm:w-auto min-w-[140px] h-[40px] bg-[#3A82CE] border border-[#C2DCF3] rounded-[8px] hover:bg-[#3272b5] transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
            <Loader2 className="w-4 h-4 text-white animate-spin" />
          ) : (
            <span className="font-inter font-medium text-[14px] leading-[15px] text-[#FFFFFF]">
              Start planning
            </span>
          )}
        </button>
