        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div className="w-[266px] h-fit bg-[#F5F5F5] p-[16px] rounded-[16px] flex flex-col gap-[10px] border border-[#EEEEEE]">
              ...
              {selectedFilters.length > 0 && (<div className="flex flex-col gap-3 mb-4 border-b border-gray-300 pb-4"><div className="flex justify-between items-center"><h4 className="font-Inter font-[700] text-[16px] leading-[100%] text-[#041830]">Your filters</h4><button onClick={clearAllFilters} className="text-[12px] text-gray-500 hover:text-red-500 hover:underline">Clear</button></div><div className="flex flex-col gap-2">{selectedFilters.map((filter) => (<button key={`selected-${filter}`} onClick={() => toggleFilter(filter)} className="w-full text-left flex items-center gap-[8px] group cursor-pointer"><div className="w-[16px] h-[16px] rounded-full bg-[#3A82CE] flex items-center justify-center flex-shrink-0"><Check size={10} className="text-white" strokeWidth={4} /></div><span className="font-Inter font-[700] text-[12px] leading-[100%] text-[#3A82CE] truncate">{filter}</span></button>))}</div></div>)}
