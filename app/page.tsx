...
// ==========================================
// 1. MAPPINGS (COPY FROM EXPLORE PAGE)
// ==========================================

// Mapping สำหรับ Filter Keywords (ใช้ช่วยหาหมวดหมู่ถ้า Tag ไม่ตรงเป๊ะ)
...
// Mapping สำหรับ Display (แสดงผลหน้าจอ)
              ...
              <div className="relative w-full h-[413px] bg-black overflow-hidden group flex-shrink-0">
                ...
                >
                  {displaySlides.map((slide) => {
                    ...
                    // ===============================================
                    // ✅ START: Logic การแสดงผล Category แบบหน้า Explore
                    // ===============================================
                    const rawTags = slide.category_tags || [];
                    const displayCategories: string[] = [];

                    rawTags.forEach(tag => {
                      const lowerTag = tag?.toLowerCase().trim();
                      // 1. เช็คจาก Map โดยตรง
                      if (CATEGORY_DISPLAY_MAP[lowerTag]) {
                        displayCategories.push(CATEGORY_DISPLAY_MAP[lowerTag]);
                        return;
                      }
                      // 2. เช็คว่ามี Keyword ซ่อนอยู่ไหม (Partial Match Word Boundary)
                      let foundMatch = false;
                      for (const [displayTitle, keywords] of Object.entries(CATEGORY_MATCHING_KEYWORDS)) {
                        // เช็คว่ามี keyword เป็น "คำ" (Word) ใน tag หรือไม่
                      ...
                      // 3. ถ้าไม่เจออะไรเลย ให้ใช้ชื่อเดิม (แทน _ ด้วย space)
                      if (!foundMatch) {
        ...
        <div className="w-[1128px] h-[426px] mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[24px]">
            {topAttractions.slice(0, 4).map((place) => {
              ...
              // ✅ ใช้ Logic เดิมซ้ำสำหรับการ์ดด้านล่าง (เพื่อให้แสดงผลเหมือนกัน)
...
`}</style>
                    ...
                    <div className="w-full h-[87px] flex flex-col gap-[4px] min-w-0">
                      ...
                      {/* ✅ ใช้ตัวแปร displayString ที่คำนวณไว้ข้างบน */}
